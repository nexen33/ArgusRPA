...
import { Settings, Trash2, Github, FileText, CheckCircle2, FolderPen } from 'lucide-react'
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden" style={{ backgroundColor: 'var(--bg-main)' }}>
      ...
      {loading ? (
        ...
        <div className="flex-1 px-3 pb-6 overflow-hidden flex flex-col animate-in fade-in duration-200">
          <div className="grid grid-cols-2 gap-4 flex-1 min-h-0 h-full" style={{ gridTemplateRows: 'auto auto minmax(0, 1fr)' }}>
          
          {/* Row 1, Col 1: Auto Launch */}
          <div style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border)' }} className="border rounded-xl p-6 flex items-center justify-between shadow-sm h-[140px]">
            <div>
          ...
          {/* Row 1, Col 2: Theme */}
          <div style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border)' }} className="border rounded-xl p-6 shadow-sm h-[140px] flex flex-col justify-center">
          ...
          <div style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border)' }} className="border rounded-xl p-6 flex flex-col justify-center shadow-sm h-[140px]">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>🖥 无头模式</h2>
                <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>关闭时显示浏览器窗口，用于可视化调试</p>
              </div>
              <button 
            ...
            <div className="flex flex-col mt-4 min-h-[38px]">
              <div 
                className={`p-2 rounded-xl text-sm flex items-center gap-2 border transition-opacity duration-200 ${headlessMode ? 'opacity-0 invisible' : 'opacity-100 visible'}`} 
          ...
          {/* Bottom Left Column: Path & About */}
          <div className="flex flex-col gap-4 h-full min-h-0">
            {/* Global Path */}
            {hasOtherDrives && (
              <div style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border)' }} className="border rounded-xl p-6 flex items-center justify-between shadow-sm shrink-0">
                <div>
                  <h2 className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>📂 默认存储路径</h2>
                  <p className="text-sm mt-1 font-mono break-all" style={{ color: 'var(--text-muted)' }}>当前: {argusIssuePath}</p>
                  <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>更改路径后，将在此目录下帮您强制创建Argus_issue及其附属文件夹</p>
                </div>
                <button 
                  onClick={handleChangePath}
                  className="px-4 py-2 rounded-xl font-bold text-sm transition-opacity hover:opacity-80 flex items-center gap-1 shrink-0 ml-4"
            ...
            {/* About */}
            <div style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border)' }} className="border rounded-xl p-6 flex flex-col shadow-sm flex-1 min-h-0 relative">
          ...
          {/* Row 3, Col 2: Changelog & Check Update */}
