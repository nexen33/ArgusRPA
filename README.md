...
**试用政策说明**：为了维持本项目的长久研发与底层运维，当前提供的所有安装包均为全功能解锁的 **7 天 Trial 试用版**。当试用期结束后，软件会自动平滑过渡至 **Pro 专业版** 锁定状态，届时将需要持有有效的授权密钥方可继续执行您的自动化流转任务。感谢您对独立开发者的理解。（授权密钥可以从开头的支持链接处获取）
